...
export default function SongsPage() {
  ...
  const search = () => {
    fetch(`http://${config.server_host}:${config.server_port}/search_songs?title=${title}` +
      ...
      .catch(err => {
        ...
        alert('Something went wrong while fetching songs. Please try again.');
      });
  }
  ...
  const surpriseMe = () => {
    const randomRange = () => {
      ...
      const high = Math.min(1, low + 0.5 + Math.random() * 0.3);
      return [low, high];
    };
    const randDance = randomRange();
    const randEnergy = randomRange();
    const randValence = randomRange();

    setDanceability(randDance);
    setEnergy(randEnergy);
    setValence(randValence);

    fetch(`http://${config.server_host}:${config.server_port}/search_songs?` +
      `danceability_low=${randDance[0]}&danceability_high=${randDance[1]}` +
      `&energy_low=${randEnergy[0]}&energy_high=${randEnergy[1]}` +
      `&valence_low=${randValence[0]}&valence_high=${randValence[1]}`
    )
      .then(res => res.json())
      .then(resJson => {
        const songsWithId = resJson.map((song) => ({ id: song.song_id, ...song }));
      ...
      .catch(() => alert('Surprise search failed.'));
  };
