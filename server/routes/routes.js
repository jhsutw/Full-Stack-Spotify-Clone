...
types.setTypeParser(20, val => Number.parseInt(val, 10)); //DO NOT DELETE THIS
...
const author = async function(req, res) {
  ...
  const name = 'Joseph Hsu';
  const pennkey = 'jsheon';

  // checks the value of type in the request parameters
  // note that parameters are required and are specified in server.js in the endpoint by a colon (e.g. /author/:type)
  if (req.params.type === 'name') { // (1) if URL parameter = .../author/name, req.params.type equals 'name' ({"data":"Joseph Hsu"})
    // res.json returns data back to the requester via an HTTP response
    res.json({ data: name });
  } else if (req.params.type === 'pennkey') { // (2) if URL parameter = .../author/name, req.params.type equals 'pennkey' ({"data":"jsheon"})
...
const song = async function(req, res) {
  ...
  console.log('Fetching song:', song_id);
  ...
  connection.query(`
    ...
    if (err) {
      ...
      console.log('Song found:', data.rows[0]);
...
const album = async function(req, res) {
  ...
  console.log('Fetching album:', album_id);
  ...
  `, [album_id], (err, data) => {
    if (err) {
      ...
      console.log('Album found:', data.rows[0]);
      res.json(data.rows[0]);
    }
  });
}
...
const albums = async function(req, res) {
  ...
  `, (err, data) => {
    if (err) {
      ...
      console.log('Albums found:', data.rows[0]);
      res.json(data.rows);
    }
  });
}
...
const album_songs = async function(req, res) {
  ...
  const album_id = req.params.album_id;
  console.log('Fetching songs for album:', album_id);
  ...
  `, [album_id], (err, data) => {
    if (err) {
      ...
      console.log('Songs found:', data.rows[0]);
      res.json(data.rows);
    }
  });
}
...
const top_songs = async function(req, res) {
  // TODO (TASK 8): use the ternary (or nullish) operator to set the pageSize based on the query or default to 10
    const page = req.query.page ? Number.parseInt(req.query.page) : null;
    const pageSize = req.query.page_size ? Number.parseInt(req.query.page_size) : 10;
...
const search_songs = async function(req, res) {
  ...
  const durationLow = req.query.duration_low ? Number(req.query.duration_low) : 60;
  const durationHigh = req.query.duration_high ? Number(req.query.duration_high) : 660;
  const playsLow = req.query.plays_low ? Number(req.query.plays_low) : 0;
  const playsHigh = req.query.plays_high ? Number(req.query.plays_high) : 1100000000;
  const danceLow = req.query.danceability_low ? Number(req.query.danceability_low) : 0;
  const danceHigh = req.query.danceability_high ? Number(req.query.danceability_high) : 1;
  const energyLow = req.query.energy_low ? Number(req.query.energy_low) : 0;
  const energyHigh = req.query.energy_high ? Number(req.query.energy_high) : 1;
  const valenceLow = req.query.valence_low ? Number(req.query.valence_low) : 0;
  const valenceHigh = req.query.valence_high ? Number(req.query.valence_high) : 1;
  const explicitParam = req.query.explicit;

  let explicitCondition = '';
  if (explicitParam !== 'true') {
    explicitCondition = 'AND explicit = 0';
  }
...
const entrance_songs = async function(req, res) {
  ...
  const maxEnergy = req.query.max_energy ? Number.parseFloat(req.query.max_energy) : 0.5;
  const minDanceability = req.query.min_danceability ? Number.parseFloat(req.query.min_danceability) : 0.73;
  ...
  connection.query(query, [maxEnergy, minDanceability, limit], (err, data) => {
    if (err) {
      console.log(err);
      res.json([]);
    } else {
